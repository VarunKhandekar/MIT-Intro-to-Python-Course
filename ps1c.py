...
while abs(current_savings - downpayment_required)>100:
    #test the lower bound; naturally in the base case, it should not hold and we move on to tet the upper bound 
    portion_saved = LowerBound/10000
    months = 0.0
    current_savings = 0.0
    salary = ann_salary
    while months < desired_months:
        ...
        #all other cases
            ...
            current_savings += current_savings*monthly_rate_of_return
            current_savings += monthly_saved
    #break here if we're close enough to the required downpayment        
    ...
    portion_saved = UpperBound/10000
    months = 0.0
    current_savings = 0.0
    salary = ann_salary
    ...
    #Set a mid point as per a bisection iteration
    MidB = int(round((UpperBound+LowerBound)/2))
    #exit if our mid point is the same as what we got from the previous iteration
    #if it is, it would suggest that we are unable to get to a % to save for the downpayment in the desired months
    if MidB == oldMidB:
        print("It is not possible to save for the downpayment in desired number of months")
        break
    #reset the oldMidB in case needed for the next iteration's above two lines' test
    oldMidB = MidB

    #test the mid point to see if it should become the new upper or lower bound
    ...
    if current_savings - downpayment_required < 0:
        ...
        UpperBound = MidB
    #We've now successfully reached the bottom if we get to here. This suggests one step in
    #the bisection iteration has been carried out
    steps += 1
